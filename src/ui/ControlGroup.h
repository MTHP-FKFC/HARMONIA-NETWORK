...
public:
  ...
  ~ControlGroup() override { removeMouseListener(this); }
  ...
  void paint(juce::Graphics &g) override {
    ...
    g.drawText(groupTitle, bounds.removeFromTop(headerHeight),
               juce::Justification::topLeft, true);

    // Цвет рамки зависит от ховера
    float alpha = isHovered ? 0.8f : 0.3f; // Ярче при наведении
    g.setColour(color.withAlpha(alpha));
    g.drawRoundedRectangle(bounds.expanded(-1.0f), 4.0f,
                           isHovered ? 2.0f : 1.0f); // Толще при наведении
