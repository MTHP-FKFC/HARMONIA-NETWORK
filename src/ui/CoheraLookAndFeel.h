...
const juce::Colour kBackground =
    juce::Colour::fromFloatRGBA(0.07f, 0.07f, 0.08f, 1.0f); // Deep Space
const juce::Colour kPanel =
    juce::Colour::fromFloatRGBA(0.11f, 0.11f, 0.13f, 1.0f); // Matte Metal
const juce::Colour kPanelLight =
    juce::Colour::fromFloatRGBA(0.16f, 0.16f, 0.18f, 1.0f); // Highlight

const juce::Colour kOrangeNeon =
    juce::Colour::fromFloatRGBA(1.0f, 0.6f, 0.0f, 1.0f); // Drive Glow
const juce::Colour kCyanNeon =
    juce::Colour::fromFloatRGBA(0.0f, 0.9f, 1.0f, 1.0f); // Net Glow
const juce::Colour kRedNeon =
    juce::Colour::fromFloatRGBA(1.0f, 0.2f, 0.3f, 1.0f); // Mute
const juce::Colour kAccentGreen =
    juce::Colour::fromFloatRGBA(0.2f, 1.0f, 0.4f, 1.0f); // System Status
...
public:
  ...
  void drawRotarySlider(juce::Graphics &g, int x, int y, int width, int height,
    ...
    // Расширяем bounds вниз для лейбла, но сохраняем оригинальную высоту для
    // ручки
    ...
    float radius =
        juce::jmin(knobBounds.getWidth(), knobBounds.getHeight()) / 2.0f;

    // --- 1. ЦВЕТОВАЯ ЛОГИКА ---
    juce::Colour mainColor = kOrangeNeon; // Default Saturation

    if (slider.getName().containsIgnoreCase("Net") ||
        slider.getName().containsIgnoreCase("Ghost")) {
      mainColor = kCyanNeon;
    } else if (slider.getName().containsIgnoreCase("Punch")) {
      mainColor = juce::Colour::fromFloatRGBA(1.0f, 0.3f, 0.5f,
                                              1.0f); // Hot Pink for Punch
    ...
    // Уменьшаем высоту bounds для самой ручки, чтобы оставить место тексту
    // снизу
    radius -=
        11.0f; // Отступ для текста (было 10, теперь 11 для большего места)
    ...
    juce::ColourGradient knobGrad(kPanelLight, center.x, center.y - knobRadius,
                                  kPanel.darker(0.4f), center.x,
    ...
    p.applyTransform(
        juce::AffineTransform::rotation(toAngle).translated(center));
    ...
    int labelHeight = 18;
    juce::Rectangle<int> nameRect(fullBounds.getX() - 2,
                                  fullBounds.getBottom() - labelHeight - 3,
                                  fullBounds.getWidth() + 4, labelHeight);

    juce::String name = slider.getName().toUpperCase();
    if (name.contains("TONE_"))
      name = name.replace("TONE_", "");
    if (name.contains("NET_"))
      name = name.replace("NET_", "");
    if (name.contains("ANALOG_"))
      name = name.replace("ANALOG_", "");

    g.drawFittedText(name, nameRect, juce::Justification::centred, 1);

    // 2. ТЕКУЩЕЕ ЗНАЧЕНИЕ (Внутри ручки или сверху при наведении)
    // Рисуем значение ВНУТРИ ручки, если она большая, или СВЕРХУ, если
    // маленькая
  ...
  void drawPopupMenuBackground(juce::Graphics &g, int width,
    ...
    g.fillAll(kBackground.darker(0.2f).withAlpha(0.95f));
  ...
  void drawPopupMenuItem(juce::Graphics &g, const juce::Rectangle<int> &area,
    ...
    if (isHighlighted) {
      g.setColour(kOrangeNeon.withAlpha(0.2f));
      g.fillRect(bounds);

      // Полоска слева
      g.setColour(kOrangeNeon);
    ...
    if (isTicked) {
      g.setColour(kOrangeNeon);
      // Fix: Make a copy of area because it's const
      auto tickArea = area;
      auto tickBounds = tickArea.removeFromRight(20).reduced(6).toFloat();
      juce::Path p;
      p.addEllipse(tickBounds);
      g.fillPath(p);
    }
  }
  ...
  void drawButtonBackground(juce::Graphics &g, juce::Button &button,
    ...
    if (button.getName() == "S") {
      if (isOn)
        baseColor = juce::Colours::yellow.darker(0.2f);
    } else if (button.getName() == "M") {
      if (isOn)
        baseColor = kRedNeon.darker(0.2f);
