...
public:
    ...
    void setLatencySamples(float samples)
    {
        ...
        // Log latency set for diagnostics
        // Round to nearest integer sample when programming the delay line
        float samplesRounded = (float) std::lround(samples);
        juce::String mixLine = juce::String::formatted("[MixEngineDiag] Setting dry delay to %.0f samples (requested %.3f)\n", samplesRounded, samples);
        ...
        dryDelayLine.setDelay(samplesRounded);
    }
