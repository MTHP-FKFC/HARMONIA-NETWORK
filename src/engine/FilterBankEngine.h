...
public:
    void prepare(const juce::dsp::ProcessSpec& spec)
    {
        ...
        fbConfig.phaseMode = FilterPhaseMode::LinearFIR256; // Максимальное качество
        ...
        // Используем TPT фильтры (State Variable), они отлично звучат и стабильны при модуляции
    ...
    {
        filterBank.reset();
        for (auto& engine : bandEngines) engine.reset();
    ...
    // Основной процесс: Вход -> Фильтр -> Сплит -> Обработка -> Сумма -> Фильтр -> Выход
    // ioBlock: входной сигнал, который будет перезаписан результатом
    // netModulations: массив из 6 значений (0..1) от NetworkManager
    void process(juce::dsp::AudioBlock<float>& ioBlock,
                 ...
                 const std::array<float, kNumBands>& netModulations)
    {
        ...
        for (int ch = 0; ch < numChannels; ++ch) {
            juce::FloatVectorOperations::copy(
                inputWrapper.getWritePointer(ch),
        ...
        for (int b = 0; b < kNumBands; ++b)
        {
            ...
            bandEngines[b].process(subBlock,
                                   params,
                                   kBandTilt[b],
                                   netModulations[b]);
        }
...
private:
    ...
    std::vector<juce::AudioBuffer<float>*> bandBufferPtrs; // Для передачи в filterBank
