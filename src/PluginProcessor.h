...
public:
  ...
  float getNetworkInput() const {
    return processingEngine.getInputRMS();
  } // SENS reacts to input level
  float getModIntensity() const {
    return lastTransientLevel.load();
  } // DEPTH reacts to modulation activity
...
private:
  ...
  // Используем std::array для жесткого ограничения (6 полос), никакого malloc в
  // аудио-потоке!
  ...
  std::array<juce::dsp::StateVariableTPTFilter<float>, 2>
      preFilters; // Tighten (HPF)
  std::array<juce::dsp::StateVariableTPTFilter<float>, 2>
      postFilters; // Smooth (LPF)
  ...
  std::array<float, kNumBands>
      gainReduction; // 1.0 = no change, <1.0 = ducked, >1.0 = boosted
  ...
  // Thread safety for state loading
